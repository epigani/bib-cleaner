...
def preprocess_bib_file(content):
    ...
    inside_entry = False
    current_key = None
    counter_removed_entries = 0
    counter_cleaned_entries = 0
    ...
    return ''.join(sorted_entries), counter_removed_entries, counter_cleaned_entries
...
def main():
    ...
    if uploaded_files:
        ...
        if st.button("Process Files"):
            processed_content, counter_removed_entries, counter_cleaned_entries = preprocess_bib_file(combined_content)
            st.write(f"Removed {counter_removed_entries} duplicate entries and cleaned {counter_cleaned_entries} entries.")
